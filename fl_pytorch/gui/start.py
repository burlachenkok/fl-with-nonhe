...
# ======================================================================================================================
compressorTypes = ["ident", "randk", "permk",
                   "bernulli", "natural",
...
#            send_scalars_to_master.append(client_stats['send_scalars_to_master'])
    ...
    if "he" in H:
        simulations_stats[job_id]["he"] = H["he"]
    if "use_relin_key_for_he" in H:
        simulations_stats[job_id]["use_relin_key_for_he"] = H["use_relin_key_for_he"]
    if "he_client_context_public_for_master_in_bytes" in H:
        simulations_stats[job_id]["he_client_context_public_for_master_in_bytes"] = H["he_client_context_public_for_master_in_bytes"]
    if "he_client_context_negotiate_by_clients_in_bytes" in H:
        simulations_stats[job_id]["he_client_context_negotiate_by_clients_in_bytes"] = H["he_client_context_negotiate_by_clients_in_bytes"]
    if "aes" in H:
        simulations_stats[job_id]["aes"] = H["aes"]
    if "aes_key_size_in_bits" in H:
        simulations_stats[job_id]["aes_key_size_in_bits"] = H["aes_key_size_in_bits"]
...
def simulationStart(H):
    ...
    if "last_round_elapsed_sec" in H:
        stats["last_round_elapsed_sec"] = H["last_round_elapsed_sec"]

    if "he" in H:
        stats["he"] = H["he"]
    if "use_relin_key_for_he" in H:
        stats["use_relin_key_for_he"] = H["use_relin_key_for_he"]
    if "he_client_context_public_for_master_in_bytes" in H:
        stats["he_client_context_public_for_master_in_bytes"] = H["he_client_context_public_for_master_in_bytes"]
    if "he_client_context_negotiate_by_clients_in_bytes" in H:
        stats["he_client_context_negotiate_by_clients_in_bytes"] = H["he_client_context_negotiate_by_clients_in_bytes"]
    if "aes" in H:
        stats["aes"] = H["aes"]
    if "aes_key_size_in_bits" in H:
        stats["aes_key_size_in_bits"] = H["aes_key_size_in_bits"]
...
def currentCommandLineFromGUI():
    ...
    if configView.cbxWeightsCentralizedConsistency.checkState() == Qt.Checked:
        cmdline.append("--weights-as-centralized")
...
def onPlotResults(export_legend_to_file):
    ...
    for g, job_id in enumerate(job_ids):
        ...
        send_scalars_from_master = [0.0] * rounds
        ...
        one_scalar_in_bits = 8 * H["bytes_per_tensor_component"]

        if True:
            # Normalize send information from clients by number of local iterations
            normalization_send_coeff = H["args"].number_of_local_iters
        else:
            normalization_send_coeff = 1.0

        # Compute aggregated statistics
        for round in range(rounds):
            round_info = H['history'][round]
            send_scalars_from_master[round] = round_info["send_scalars_from_master"]
            ...
            for client_id, client_summary in round_info['client_states'].items():
                ...
                send_scalars_to_master[round] += float(client_stats['send_scalars_to_master']) / normalization_send_coeff
                send_scalars_to_master_div_clients[round] += float(client_stats['send_scalars_to_master']) / float(num_clients_in_round) / normalization_send_coeff
        ...
        for round in range(rounds):
            ...
            for axis_type, axis_values in ( (xAxisType, X), (yAxisType, Y) ):
                ...
                if axis_type == "# of bits from master":
                    axis_values[round] = send_scalars_from_master[round] * one_scalar_in_bits
        ...
        plot_args["compute_type"] = H['args'].compute_type

        plot_args["he"] = ""
        plot_args["aes"] = ""
        plot_args["aes_key_size_in_bits"] = ""
        plot_args["he_client_context_public_for_master_in_bytes"] = ""
        plot_args["he_client_context_negotiate_by_clients_in_bytes"] = ""
        plot_args["use_relin_key_for_he"] = ""

        if "aes" in H and H["aes"]:
            plot_args["aes"] = "AES"
            plot_args["aes_key_size_in_bits"] = H["aes_key_size_in_bits"]

        if "he" in H and H["he"]:
            plot_args["he"] = "HE/CKKS"
            plot_args["he_client_context_public_for_master_in_bytes"] = H["he_client_context_public_for_master_in_bytes"]
            plot_args["he_client_context_negotiate_by_clients_in_bytes"] = H["he_client_context_negotiate_by_clients_in_bytes"]
            plot_args["use_relin_key_for_he"] = H["use_relin_key_for_he"]
        # =================================================================================================================
    ...
    if analysisView.cbxLegendLogScalingY.checkState() == Qt.Checked and doNotAggregate:
        for p in plots:
            ...
            ax.semilogy(p["X"], p["Y"], marker = p["marker"], markevery = p["markevery"], linestyle = p["linestyle"], label = formatedLabel, color = color[(rasterizedPlots + 3) % len(color)])
            rasterizedPlots += 1
