...
#TODO CHANGE THIS
from utils.hf_utils import compute_wer
...
def initialise_model(model_name, dataset, dataset_ref, args, resume_from=None, load_best=None, use_pretrained=False):
    ...
    if model_name == "wav2vec2_base":
        model = models.wav2vec2_base(pretrained=use_pretrained)

    if model_name == "wav2vec2_large":
        model = models.wav2vec2_large(pretrained=use_pretrained)    
...
def local_training(thread, client_state, client_id, msg, model_dict_original, optimiser_dict_original,
    ...
    optimiser_dict_found = algorithms.findRecentRecordAndRemoveFromHistory(client_state['H'], client_id, "optimiser")
    ...
    buffers_found = algorithms.findRecentRecordAndRemoveFromHistory(client_state['H'], client_id, "buffers")
...
def run_one_communication_round(H, model, train_loader, criterion, local_optimiser, optimiser_dict_original,
    ...
    while i < len(sampled_clients_round):
        ...
        if exec_ctx.local_training_threads.workers() == 0:
            ...
            if H["args"].weights_as_centralized:
                client_state["weight"] = client_data_samples
...
def train_model(client_state, model, train_loader, criterion, local_optimiser, device, round,
    ...
    # Make evaluation
    if "not_evaluate" not in experimental_options:
...
def compute_loss(model, criterion, output, label):
    ## This needs refactoring A LOT in terms of getting input_lengths but for Base model this is sufficient
    if type(criterion) is torch.nn.CTCLoss:
        input_lengths = torch.ones([output[0].shape[0],output[0].shape[1]],dtype=torch.long).to(output[0].device).sum(-1)
        labels_mask = label >= 0
        target_lengths = labels_mask.sum(-1)
        flattened_targets = label.masked_select(labels_mask)
        log_probs = nn.functional.log_softmax(output[0], dim=-1, dtype=torch.float32).transpose(0, 1)
...
def update_metrics(metrics_meter, loss, batch_size, output, label):
    ...
    if type(output) is tuple:
        word_error_rate = compute_wer(output,label)
        metrics_meter["top_1_acc"].update(word_error_rate,batch_size)
    elif hasattr(output,'shape') and output.shape[1] >= 5:
