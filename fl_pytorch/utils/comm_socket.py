...
class CommSocket:
    ...
    def rawSend(self, msg):
        ...
        print(f"  >>> SEND BYTES: {totalsent/1024.0/1024.0} MBytes.")
    ...
    def rawRecv(self):
        ...
        while bytes_recd < MSGLEN:
            chunk = self.sock.recv(min(MSGLEN - bytes_recd, 3*1024*1024*1024))
            if chunk == b'':
                raise RuntimeError("socket connection broken")
            chunks.append(chunk)
            bytes_recd = bytes_recd + len(chunk)

        print(f"  >>> RECEIVED BYTES: {bytes_recd/1024.0/1024.0} MBytes.")
