...
from utils.model_funcs import get_training_elements, evaluate_model, get_lr_scheduler, get_optimiser, \
    run_one_communication_round, local_training, initialise_model, model_to_device
...
def main(args, raw_cmdline, extra_):
    ...
    # Test set dataloader
    if args.model.startswith('wav2vec2'):
        testloader = torch.utils.data.DataLoader(testset,
                                             ...
                                             pin_memory=False,
                                             drop_last=False,
                                             collate_fn=testset.get_collate_func(args.model)
                                             )
    else:
        testloader = torch.utils.data.DataLoader(testset,
                                                batch_size=test_batch_size,
    ...
    if not args.evaluate:  # Training mode
        if args.model.startswith('wav2vec2'):
            trainloader = torch.utils.data.DataLoader(
                ...
                pin_memory=False,
                drop_last=False,
                collate_fn=trainset.get_collate_func(args.model))
        else:
            trainloader = torch.utils.data.DataLoader(
                trainset,
                batch_size=args.batch_size,
        ...
        for th in exec_ctx.local_training_threads.threads:
            if args.model.startswith('wav2vec2'):
                ...
                if hasattr(trainset,'dataset'):
                    used_dataset = trainset.dataset
                    trainset.dataset = None
                    th.trainset_copy = copy.deepcopy(trainset)
                    th.trainset_copy.dataset = used_dataset
                    trainset.dataset = used_dataset
                else:
                    th.trainset_copy = copy.deepcopy(trainset)                    
                ...
                th.train_loader_copy = torch.utils.data.DataLoader(th.trainset_copy,
                                                                ...
                                                                pin_memory=False,
                                                                drop_last=False,
                                                                collate_fn=th.trainset_copy.get_collate_func(args.model))     
            else:
                th.trainset_copy = copy.deepcopy(trainset)
                th.train_loader_copy = torch.utils.data.DataLoader(th.trainset_copy,
                                                                batch_size=args.batch_size,
        ...
        for th in exec_ctx.eval_thread_pool.threads:
            if args.model.startswith('wav2vec2'):
                ...
                if hasattr(testset,'dataset'):
                    used_dataset = testset.dataset
                    testset.dataset = None
                    th.testset_copy = copy.deepcopy(testset)
                    th.testset_copy.dataset = used_dataset
                    testset.dataset = used_dataset
                else:
                    th.testset_copy = copy.deepcopy(testset)    
                ...
                th.testloader_copy = torch.utils.data.DataLoader(th.testset_copy,
                                                                ...
                                                                pin_memory=False,
                                                                drop_last=False,
                                                                collate_fn=th.testset_copy.get_collate_func(args.model))   
            else:            
                th.testset_copy = copy.deepcopy(testset)
                th.testloader_copy = torch.utils.data.DataLoader(th.testset_copy,
                                                                batch_size=test_batch_size,
...
def makeBackupOfServerState(H, round):
    ...
    he_client_context = None
    he_client_context_public = None

    if "he_client_context" in H:
        he_client_context = H["he_client_context"]
        H["he_client_context"] = None

    if "he_client_context_public" in H:
        he_client_context_public = H["he_client_context_public"]
        H["he_client_context_public"] = None
    ...
    if "he_client_context" in H:
        H["he_client_context"] = he_client_context

    if "he_client_context_public" in H:
        H["he_client_context_public"] = he_client_context_public
...
def init_and_train_model(args, raw_cmdline, trainloader, testloader, exec_ctx):
    ...
    for th in exec_ctx.local_training_threads.threads:
        # ADDED TO TACKLE DEEPCOPY ISSUE
        if hasattr(model,'wav2vec2'):
            # Detach weight Norm
            model.wav2vec2.encoder.pos_conv_embed.conv.weight = torch._weight_norm(model.wav2vec2.encoder.pos_conv_embed.conv.weight_v, model.wav2vec2.encoder.pos_conv_embed.conv.weight_g , 2).detach()

            th.model_copy = copy.deepcopy(model)
            #reattach
            model.wav2vec2.encoder.pos_conv_embed.conv.weight = torch._weight_norm(model.wav2vec2.encoder.pos_conv_embed.conv.weight_v, model.wav2vec2.encoder.pos_conv_embed.conv.weight_g , 2)
            th.model_copy.wav2vec2.encoder.pos_conv_embed.conv.weight = torch._weight_norm(th.model_copy.wav2vec2.encoder.pos_conv_embed.conv.weight_v, th.model_copy.wav2vec2.encoder.pos_conv_embed.conv.weight_g , 2)
        else:
            th.model_copy = copy.deepcopy(model)
    ...
    for i in range(args.rounds):
        ...
        if (i % args.eval_every == 0 or i == (args.rounds - 1)) and metric_to_optim != "none":
            ...
            # ==========================================================================================================
            if "not_evaluate" not in exec_ctx.experimental_options:
                # Evaluate model
        ...
        H['history'][current_round]['send_scalars_from_master'] = H['send_scalars_from_master']
