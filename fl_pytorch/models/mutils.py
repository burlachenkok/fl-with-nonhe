...
def mark_compute_partial_derivatives_for(model: torch.nn.Module, indicies4mark, skipFrozen: bool = True):
    """
    ...
    Returns:
        torch.Tensor: all parameters in a form of a tensor
    """
    if type(indicies4mark) == torch.Tensor:
        ind = set(indicies4mark.tolist())
    else:
        ind = indicies4mark

    turned_off_tensor = set()
    ...
    for p in model.parameters():
        ...
        add_tensor_to_turn_off = True

        numel = p.numel()
        for i in range(numel):
            if currentInd + i not in ind:
                add_tensor_to_turn_off = False
                break

        currentInd += numel

        if add_tensor_to_turn_off:
            turned_off_tensor.add(p)
            p.requires_grad_(False)

    return turned_off_tensor

def unmark_compute_partial_derivatives_for(indicies4unmark):
    """
    Get all model gradient data as a single dense vector.
    ...
    Returns:
        torch.Tensor: all parameters in a form of a tensor
    """
    for tensor2unmark in indicies4unmark:
        tensor2unmark.requires_grad_(True)
